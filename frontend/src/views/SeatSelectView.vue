<template>
  <div class="selectSeatDiv">
    ...
    <b-row>
      ...
      <b-col>
        <p>
          CGV 구미 | {{ seatData.theater }} | 남은좌석
          {{
            seatData.maxcol * seatData.maxrow - seatData.selectedCol.length
          }}
          / {{ seatData.maxcol * seatData.maxrow }}
        </p>
        <h3>
          2022.06.14 (화) {{ this.$route.params.screenTime }} ~
          {{ calcTime(this.$route.params.screenTime) }}
        </h3>
      </b-col>
    </b-row>
    <b-row>
    ...
    <b-row>
      <b-col>
        <h5 style="text-align: right" v-if="selectSeat.length != 0">
          일반 : {{ price }}원 X {{ selectSeat.length }}
        </h5>
        <h5 style="text-align: right" v-if="selectSeat.length != 0">
          총 금액 : {{ price * selectSeat.length }}원
        </h5>
  ...
  methods: {
    ...
    chooseSeat(row, col) {
      for (var key in this.selectSeat) {
        if (
          this.selectSeat[key].row == row &&
          this.selectSeat[key].col == col
        ) {
  ...
  computed: {
    seatTable() {
      ...
      for (var key in this.seatData.selectedRow) {
        table[this.seatData.selectedRow[key].charCodeAt(0) - 65][
          this.seatData.selectedCol[key]
        ] = 1;
      }
